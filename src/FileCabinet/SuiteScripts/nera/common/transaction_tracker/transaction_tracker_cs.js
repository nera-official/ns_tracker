...
define(["N/record", "N/query", "N/log", "N/ui/message", "N/search"], function (
  record,
  query,
  log,
  message,
  ...
  function queryTracker(recordType, recordId) {
    console.log("Record Type: ", recordType, "Record Id:", recordId);
    try {
      // Check if recordId is empty, null, or undefined
      if (!recordId) {
        console.log("Info", "RecordId is empty. Skipping tracker query...");
        return [];
      }
      const trackerObj = search.create({
        ...
        filters: [
          ...
          "AND",
          ["custrecord_tt_linked_transaction", "is", recordId],
        ],
        columns: [
